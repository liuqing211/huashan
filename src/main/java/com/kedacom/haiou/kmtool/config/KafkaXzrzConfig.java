...
@EnableKafka
@Configuration
public class KafkaXzrzConfig {

    @Value("${spring.kafka.xzrz.bootstrap-servers}")
    private String bootstrapServers;

    @Value("${spring.kafka.xzrz.group-id}")
    private String groupId;

    @Value("${spring.kafka.xzrz.producer.key-serializer}")
    private String keySerializer;

    @Value("${spring.kafka.xzrz.producer.value-serializer}")
    private String valueSerializer;

    @Value("${spring.kafka.xzrz.consumer.key-serializer}")
    private String keyDeserializer;

    @Value("${spring.kafka.xzrz.consumer.value-serializer}")
    private String valueDeserializer;

    @Bean(name = "kafkaXzrzTemplate")
    public KafkaTemplate<String, String> kafkaXzrzTemplate(){
        return new KafkaTemplate<String, String>(producerFactory());
    }

    @Bean(name = "kafkaXzrzContainerFactory")
    KafkaListenerContainerFactory<ConcurrentMessageListenerContainer<Integer, String>> kafkaXzrzContainerFactory(){
    ...
    private Map<String,Object> consumerConfigs() {
        ...
        consConfigs.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, false);
        ...
        consConfigs.put(ConsumerConfig.MAX_POLL_INTERVAL_MS_CONFIG, 864000);
        return consConfigs;
    }
